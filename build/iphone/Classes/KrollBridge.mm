...
@implementation databankSurveyorObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_databanksurveyor);
	RELEASE_TO_NIL(modules);
...
-(void)injectPatches
{
	// called to inject any databankSurveyor patches in JS before a context is loaded... nice for 
...
{
	// create databankSurveyor global object
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_databanksurveyor = [[databankSurveyorObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_databanksurveyor];
	
	NSString *_databanksurveyorNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _databanksurveyorNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_databanksurveyor valueForKey:name];
...
-(id)require:(KrollContext*)kroll path:(NSString*)path
{
	...
	@throw [NSException exceptionWithName:@"org.databanksurveyor.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
