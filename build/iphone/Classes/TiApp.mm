...
- (NSString*)userAgent
{
	if (userAgent==nil)
	{
		...
		NSString *kdatabankSurveyorUserAgentPrefix = [NSString stringWithFormat:@"%s%s%s %s%s","Appc","eler","ator","Tita","nium"];
		userAgent = [[NSString stringWithFormat:@"%@/%s (%@)",kdatabankSurveyorUserAgentPrefix,TI_VERSION_STR,currentDeviceInfo] retain];
	}
	return userAgent;
}
